...
local function checkTableJob2(key, val, xPlayer, xPlayers, minimal)
    for valIndex = 1, #val do
        local value = val[valIndex]
        if not xPlayers[value] then
            xPlayers[value] = {}
...
function ESX.GetExtendedPlayersJob2(key, val, minimal)
    if not key then
        if not minimal then
            return ESX.Table.ToArray(ESX.Players)
        end
    ...
    if type(val) == "table" then
        for _, xPlayer in pairs(ESX.Players) do
            checkTableJob2(key, val, xPlayer, xPlayers, minimal)
        end
...
function ESX.GetNumPlayersJob2(key, val)
    if not key then
        return #GetPlayers()
    end
    ...
    if type(val) == "table" then
        ...
        local filteredPlayers = ESX.GetExtendedPlayersJob2(key, val)
        for i, v in pairs(filteredPlayers) do
            numPlayers[i] = (#v or 0)
        end
        return numPlayers
    ...
    return #ESX.GetExtendedPlayersJob2(key, val)
end
...
function ESX.DoesJob2Exist(job2, grade2)
    ...
    return (ESX.Jobs2[job2] and ESX.Jobs2[job2].grades[tostring(grade2)] ~= nil) or false
end
