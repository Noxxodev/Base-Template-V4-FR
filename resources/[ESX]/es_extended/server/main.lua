...
AddEventHandler("esx:setJob2", function(_, job, lastJob)
    local lastJobKey = ("job2_%s:count"):format(lastJob.name)
    local jobKey = ("job2_%s:count"):format(job.name)
    local currentLastJob = Core.JobsPlayerCount["job2_" .. lastJob.name]

    Core.JobsPlayerCount["job2_" .. lastJob.name] = ((currentLastJob and currentLastJob > 0) and currentLastJob or 1) - 1
    Core.JobsPlayerCount["job2_" .. job.name] = (Core.JobsPlayerCount["job2_" .. job.name] or 0) + 1

    GlobalState[lastJobKey] = Core.JobsPlayerCount["job2_" .. lastJob.name]
    GlobalState[jobKey] = Core.JobsPlayerCount["job2_" .. job.name]
end)
